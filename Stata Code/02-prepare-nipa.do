...
use "01-import-nipa/nipa-monthly-series.dta", clear
...
merge 1:1 year month using "01-import-aid-covid/covid-aid-monthly.dta", nogenerate
...
use "01-import-nipa/nipa-quarterly-series.dta", clear
...
// Add data for COVID aid
merge 1:1 year quarter using "01-import-aid-covid/covid-aid-quarterly.dta", nogenerate
...
use "01-import-nipa/nipa-annual-series.dta", clear
...
merge 1:1 year using "01-import-dina/dina-full-aggregates.dta", keep(master match) ///
...
merge 1:1 year using "01-import-ici/ici-data-flows.dta", keepusing(contrib_ira_pretax) nogenerate
...
local to_disaggregate_yrly ///
    ...
    y604rc y605rc a2213c a1581c con544 con170 con550 ///
...
generate nipa_princ = cond(missing(nipa_flemp), 0, nipa_flemp) ///
    + cond(missing(nipa_proprietors), 0, nipa_proprietors) ///
    + cond(missing(nipa_rental), 0, nipa_rental) ///
    + cond(missing(nipa_corptax), 0, nipa_corptax) ///
    + cond(missing(nipa_profits), 0, nipa_profits) ///
    + cond(missing(nipa_fkfix), 0, nipa_fkfix) ///
    - cond(missing(nipa_fknmo), 0, nipa_fknmo) ///
    + cond(missing(nipa_prodtax), 0, nipa_prodtax) ///
    - cond(missing(nipa_prodsub), 0, nipa_prodsub) ///
    - cond(missing(nipa_covidsub), 0, nipa_covidsub) ///
    + cond(missing(nipa_govin), 0, nipa_govin) ///
    + cond(missing(nipa_npinc), 0, nipa_npinc)
...
rename nipa_corr* nipa_*
rename deflator nipa_deflator
rename pop nipa_pop

keep if year >= 1973

save "02-prepare-nipa/nipa-simplified-monthly.dta", replace

// Make quarterly/yearly version
generate quarter = ceil(month/3)
gcollapse (mean) nipa_*, by(year quarter)
save "02-prepare-nipa/nipa-simplified-quarterly.dta", replace

gcollapse (mean) nipa_*, by(year)
save "02-prepare-nipa/nipa-simplified-yearly.dta", replace
